...
public class Banker
{
    ...
    public void purchaseProperty(GameObject currentPlayer) {
    ...
    public void auctionProperty(GameObject refusedPlayer, PropertyData property) {
        int highestBid = 0;
    ...
    public void payRent(GameObject currentPlayer) {
    ...
    public void unableToPay(int rentToPay, GameObject currentPlayer) {
        ...
        while (rentToPay > currentPlayer.cash){
            bool found = false;
            while (!found) {
                Console.WriteLine("You are unable to pay the rent, which property would you like to sell or mortgage? ");
                string propertyConsidered = Console.ReadLine();
                for(int i = 0; i < currentPlayer.owned.Length(); i++) {
                    ...
                    if (currentPlayer.owned[i].NameProperty == propertyConsidered) {
                        ...
                        Console.WriteLine("You do not own this property, please try again.");
                    }
                }
            }

            Console.WriteLine("Would you like to mortgage or sell this property? M/S ");
            ...
            if (currentPlayer.cash < rentToPay && currentPlayer.owned.Count == 0) {
                    Console.WriteLine("You are bankrupt, please leave the game.");
                    // remove player from game
                }
            else if (currentPlayer.cash < rentToPay && currentPlayer.owned.Count > 0) {
                Console.WriteLine("You are still unable to pay the rent, please sell or mortgage another property.");
            }
        }
    }

    public void mortgageProperty(GameObject currentPlayer, PropertyData property) {
    ...
    public void unMortgageProperty(GameObject currentPlayer, PropertyData property) {
        currentPlayer.cash -= (property.Price/2);
    ...
    public void sellProperty(GameObject currentPlayer, PropertyData property) {
        ...
        if (property.NumOfHouses > 0) {
            ...
            int housesToSell = Console.ReadLine();
    ...
    public void improveProperty(GameObject currentPlayer) {
        Console.WriteLine("Would you like to improve a property? Y/N ");
        if (Input.GetKeyDown(KeyCode.y)) {
            bool found = false;
            PropertyData property = null;
            while (!found) {
                Console.WriteLine("Which property would you like to improve? ");
                string propertyConsidered = Console.ReadLine();
                for(int i = 0; i < currentPlayer.owned.Length; i++) {
                    if (currentPlayer.owned[i].NameProperty == propertyConsidered) {
            ...
            if (property.NumOfHouses >= 5) { //I want to add another variable for the number of houses 
                Console.WriteLine("You cannot improve this property any further.");
                return;
            }
            ...
            if (colourPositions.TryGetValue(property.Group, out List<int> positions)) {
                ...
                for (int i = 0; i < positions.Count; i++) {
                    ...
                    if (propertyData[positions[i]].Owner != currentPlayer) {
                        owned = false;
                        Console.WriteLine("You do not own all of the properties in this colour group, you cannot improve this property.");
                        return;
                    }
                    ...
                    if (propertyData[positions[i]].NumOfHouses < property.NumOfHouses) {
                        Console.WriteLine("You cannot improve this property to a difference of more than 1 house between each property in a set.");
                        return;
                    }
                }
            }
            ...
            Console.WriteLine("You have chosen not to improve this property.");
        }
        else {
